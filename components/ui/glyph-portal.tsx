...
export default function GlyphPortal({
  ...
  useLayoutEffect(() => {
    ...
    // Local change from the published source: the explicit type only. Without it
    // `RegExpMatchArray | never[]` collapses the filter callback's parameter to
    // `never` and `family.trim()` fails to compile. Behaviour is unchanged.
